...
p_5 <- BD_dat %>% 
  ...
  stat_summary(geom = "errorbar", aes(group = inhull, color = inhull), # plot interquartile range for conditional distributions
               ...
               show.legend = FALSE) + 
  stat_summary(geom = "point", # plot medians
  ...
  scale_alpha_manual(values = c(0, 0, 1), guide = "none") + # this alpha scaling makes conditional distributions invisible, for plotting just marginal medians
...
##### Fig 6 : convex hull probabilities #####

hp_equal <- read_csv("../simulation_results/hull_probabilities_equal_samples_1000.csv")
hp_random <- read_csv("../simulation_results/hull_probabilities_random_samples_1000.csv")

hp_equal <- hp_equal %>% mutate(type = "Equal")
hp_random <- hp_random %>% mutate(type = "Random")
hull_probabilities <- rbind(hp_equal, hp_random)

p_6 <- hull_probabilities %>%
  ggplot() + 
  aes(x = p, color = as.factor(m)) + 
  geom_line(aes(y = probability), show.legend = FALSE) +
  geom_line(aes(y = theory_prediction), linetype = "dashed", show.legend = FALSE) + # add theoretical predictions
  geom_tile(data = data.frame(p = rep(Inf, 4), y = rep(-Inf, 4), m = c(10, 15, 20, 25)), # plot dummy points to produce color bar
            aes(y = y, fill = as.factor(m))) +
  facet_grid(.~type) + 
  scale_fill_viridis_d(name = "Number of \n species") +
  scale_color_viridis_d(name = "Number of \n species") +
  xlab("Number of resources") + 
